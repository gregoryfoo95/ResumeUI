...
export default function PromptWindow({onPromptClick}: PromptWindowProps): JSX.Element {
    return (
      <div className="flex flex-col gap-y-2 px-10 font-bold ">
        <div
          className="grid w-full h-16 rounded-full px-8 border-4 border-primary bg-base-300 text-white place-content-center cursor-pointer text-sm md:text-lg"
          onClick={() => onPromptClick(ActiveComponent.AboutMe)}
        >
          Tell me more about yourself!
        </div>
        <div
          className="grid w-auto h-16 rounded-full px-8 border-4 border-primary bg-base-300 text-white place-content-center cursor-pointer text-sm md:text-lg"
